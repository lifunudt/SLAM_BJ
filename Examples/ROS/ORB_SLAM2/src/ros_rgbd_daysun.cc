...
public:
    ...
    int loop_id;//record the number of loop closures
};

/// Creat ros-related viewer
My_Viewer::ros_viewer_daysun *ros_view;
...
int main(int argc, char **argv) {
    ...
    ros_view = new My_Viewer::ros_viewer_daysun(argv[2]);
    mptViewer = new thread(&My_Viewer::ros_viewer_daysun::Run, ros_view);
...
//    mpSLAM->TrackRGBD(cv_ptrRGB->image,cv_ptrD->image,cv_ptrRGB->header.stamp.toSec());
    ...
    if (mpSLAM->isLocalOptimization()) {
        ...
        if (isRun) {
            ...
            for (std::vector<ORB_SLAM2::KeyFrame *>::const_iterator it = updateId.begin(), itEnd = updateId.end();
                 it != itEnd; it++) {
